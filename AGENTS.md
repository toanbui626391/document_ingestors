...
8. **Reliability & Mid-Process Restart**: Implement two-phase batch checkpointing and POSIX signal trapping (`SIGTERM`/`SIGINT`) to ensure workers crash and resume with zero rework.
9. **Standardized Observability**: Emit standardized Prometheus metrics, OpenTelemetry distributed trace spans, and structured JSON audit logs across all ingestion pipelines.
10. **Diagram Aesthetics & Dual-Mode Contrast**: When generating Mermaid diagrams, strictly follow the [mermaid-drawing skill](file:///c:/Users/ToanBX/dev/personal/document_ingestors/.agents/skills/mermaid-drawing/SKILL.md) to ensure high-contrast legibility across both Dark Mode and Light Mode, with thicker borders on bigger containers.
